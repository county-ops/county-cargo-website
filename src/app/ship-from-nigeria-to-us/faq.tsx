...
export function Faq() {
  ...
  return (
    <section id="faq" className="py-16 sm:py-24 bg-gray-50 border-t border-gray-100">
      <script
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 sm:mb-16" data-aos="fade-up">
          <span className="inline-block px-3 py-1 bg-blue-50 text-primary text-xs sm:text-sm font-bold tracking-wider uppercase rounded-full mb-3">
            Got Questions?
          </span>
          ...
          <p className="text-base sm:text-lg text-gray-600 max-w-2xl mx-auto">
            Everything you need to know about cargo rates, permitted foodstuffs, export documentation, and US nationwide delivery.
          </p>
        </div>
        <Accordion type="single" collapsible className="w-full space-y-4">
          {faqItems.map((item, index) => (
            <AccordionItem
              value={`item-${index}`}
              key={index}
              className="bg-white border border-gray-200 p-5 sm:p-6 rounded-xl shadow-xs transition-all hover:border-primary/40"
              data-aos="fade-up"
