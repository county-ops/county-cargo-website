...
export const usBarrelsFaqItems = [
  {
    question: 'How do I ship a barrel from the USA to Nigeria with County Cargo?',
    answer:
      'To ship a barrel from the USA to Nigeria, first contact County Cargo to request an up-to-date quotation based on your barrel size, estimated weight and destination in Nigeria. Our team will provide packing recommendations and detailed instructions for dispatching or sending your prepared barrel to our designated US processing warehouse in Texas. Once received and processed, your barrel is scheduled for shipping to Nigeria.',
  },
  {
    question: 'Can County Cargo collect my barrel anywhere in the USA?',
    answer:
      'Any local collection or third-party courier pickup across Texas, Georgia, New York, Maryland, Illinois, California, Florida and other US states is arranged on a scheduled basis or handled via domestic freight carriers. Customers can also route their barrels directly to our designated US warehouse following our provided receiving instructions.',
  },
  {
    question: 'How much does it cost to ship a barrel from the US to Nigeria?',
    answer:
  ...
  },
  {
    ...
    answer:
      'Yes, you can ship multiple barrels, large cartons, and crates together in a single consolidated consignment. Shipping multiple barrels at once is an excellent way for families, churches, community groups and businesses across the US to optimize their logistics and streamline clearance in Nigeria.',
  },
];

export function UsBarrelsFaq() {
  const faqSchema = {
    '@context': 'https://schema.org',
    '@type': 'FAQPage',
    mainEntity: usBarrelsFaqItems.map((item) => ({
      '@type': 'Question',
      name: item.question,
      acceptedAnswer: {
        '@type': 'Answer',
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12 sm:mb-16">
          ...
          <p className="text-base sm:text-lg text-gray-600 max-w-2xl mx-auto">
            Practical answers about shipping barrels, boxes and personal effects from anywhere in the USA to Nigeria.
          </p>
        </div>

        <Accordion type="single" collapsible className="w-full space-y-4">
          {usBarrelsFaqItems.map((item, index) => (
            <AccordionItem
              value={`item-${index}`}
              key={index}
